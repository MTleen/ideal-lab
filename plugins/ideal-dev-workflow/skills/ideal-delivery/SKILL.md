---
name: ideal-delivery
description: "Use when P14 wiki review is completed and the iteration needs to be finalized. Triggers on: 成果提交, 标记完成, 归档, 交付, delivery, submit, PR创建, 合并代码."
---
...
| 场景 | 处理 |
|------|------|
| `gh` CLI 未安装 | 跳过 PR 创建，手动提示用户前往 GitHub 创建 PR |
| `git push` 失败（无权限/分支保护） | 输出具体错误，让用户手动 push 或检查权限 |
| Merge 冲突 | 提示用户手动解决冲突后继续 |
| `project-config.md` 不存在 | 使用默认值：主分支 = `main`，分支前缀 = `feature/` |
| Worktree 已被手动删除 | 跳过 `git worktree remove`，仅删除本地分支 |
| Feature 分支已被删除 | 跳过清理步骤，仅归档 |
| 无远程仓库 | 跳过 Step 0/5，仅本地 commit + 归档 |
| 多个进行中迭代 | 列出所有进行中迭代，询问用户要完成哪一个 |