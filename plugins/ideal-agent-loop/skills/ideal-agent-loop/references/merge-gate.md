...
7. 标 goal status=done + quality.status=accepted → 出队下一个（不停）
```
...
```text
1. goal verified/awaiting_acceptance → 暂停 outer loop，向用户展示：
   - .agent-loop/{goal}/report.md（完成报告 + 验收证据）
   - task-boundary-matrix 全 passed 证据
   - 真机验证证据（截图/录影/playwright 或 chrome-devtools 输出）
   - quality evidence、对抗性审查 findings、reopen 记录和漏测原因
   - 即将执行：{goal_branch} → {base_branch}，merge_gate=confirm
2. 等用户确认：
   - "验收通过并合并" / "确认 accepted" → 写入 quality.status=accepted，再执行 `auto` 的 step 1-7
...
6. 标 goal status=done + quality.status=accepted → 出队下一个
```
...
- 任何 gate 失败（fast-forward 冲突 / 推送失败 / PR 被拒）→ **不标 goal done，不标 accepted**，报告卡点等用户决策（不无限重试）。
- 合并完成、worktree 清理后，才标 goal `status=done` 且保持 `quality.status=accepted`，再出队下一个 goal（下一个 goal 从更新后的 base_branch 拉新 worktree，保证基线干净）。
- 后续发现 bug、用户反馈或审查失败时，不创建孤立修复；必须 reopen 原 goal 或创建带 `linkedRequirementId` 的 bug 子需求，记录 `missingTestReason` 和 `requiredRegression`。
