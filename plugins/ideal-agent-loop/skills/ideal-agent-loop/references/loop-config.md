...
| `merge_gate` | `confirm` | `auto` / `confirm` / `pr` | goal 达到 `quality.status=accepted` 后合并 base_branch 的策略（详见 `merge-gate.md`） |
...
| gate | 行为 |
|------|------|
| `auto` | goal accepted → fast-forward 合并 base_branch → 推送 → 清理 worktree。全自动接力，不停 |
| `confirm` | goal verified/awaiting_acceptance → 暂停，展示 report/trace/task 矩阵/真机证据/质量证据 → 用户确认 accepted 后执行 `auto` 的合并清理 |
| `pr` | goal accepted → 委托 `ideal-dev-workflow/ideal-delivery` 走 PR 流程（`gh pr create` → 等合并 → 清理） |
