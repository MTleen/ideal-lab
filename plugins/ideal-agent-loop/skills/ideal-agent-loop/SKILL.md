...
loop（outer，跨 goal 接力）:
  ...
  6. goal 全 task passed + 全局审计通过 → 按 merge_gate 合并 {base_branch}
  ...
  7. 清理 goal worktree + 删分支 → goal status=done → 回 step 1（出队下一个，从更新后的 base_branch 拉）
```

**终止条件**：需求池空 / goal blocked（连续 3 次 verification 失败）/ 用户停止 / `merge_gate: confirm` 或 `pr` 时等用户合并决策（gate 设计的暂停点，非异常停止）。
