...
### Step 4: 清理本次任务的临时产物（强制）

只有在最终交付物存在、非空、可打开且内容与格式验证通过后，才能开始清理。验证失败时必须保留诊断材料，不得提前删除。

**应清理：**

- 仅用于校验 Word/PPT 的预览 PDF、渲染检查 PDF 和逐页临时 PDF。
- 已被最终交付物替代的 draft、preview、backup 和迭代版本。
- 本次渲染产生的临时合并 Markdown、临时 DOCX/PPTX、QA 缩略图、截图检查目录及转换工具临时目录。

**必须保留：**

- `最终交付物`，以及用户明确要求一并交付的 PDF 或其他格式。
- 用户提供的原始素材、参考 PDF/DOCX/PPTX、模板和其他用户文件。
- `sections/` Markdown 中间件、`images/` 配图、流程状态、需求分析、大纲和任务拆分等正式流程资产。
- 来源不明、无法确认是否由本次任务生成的文件。

**安全执行规则：**

1. 先列出候选清单，逐项确认“本次任务生成 + 已有最终替代 + 不在交付范围”。
2. 只删除确认后的精确路径；禁止对项目目录使用无约束通配符或递归清空。
3. 优先使用可恢复的删除方式；若只能永久删除，也必须逐项使用精确路径。
4. 清理后重新检查目录，确认最终交付物和必须保留项仍然存在。
5. 完成摘要必须列出已删除的临时产物；没有可安全确认的候选项时，写明未清理原因，不得猜测删除。
...
├── 临时产物清理：preview.pdf、temp_merged.md
└── 任务统计
    ├── 子任务数：7
    ├── 总字数：15234
    ├── 配图数：4
