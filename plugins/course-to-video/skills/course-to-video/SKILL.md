...
- 用户的参考资料是 PPT/Word/PDF → 先提取文本内容为 Markdown，再从 Stage 1 开始
...
用户要求修改某个阶段的**部分产物**时（如"改第 3 页旁白"、"重做第 5 页截图"）：

1. 读取已有的 JSON 文件（`slides.json` 或 `narrations.json`）
2. 仅修改指定条目，保留其余内容不变
3. 重新执行受影响的下游阶段（如修改了旁白则需重跑 Stage 4 生成该页音频，再跑 Stage 6 合成）

### 失败恢复

脚本驱动阶段（Stage 4-6）失败时：

1. 检查错误输出，判断失败原因（依赖缺失 / 网络超时 / 文件损坏）
2. 若为可重试错误（如 edge-tts 429 限流），等待后自动重试一次
3. 若为不可恢复错误，告知用户具体原因和手动修复建议，保留已成功的中间产物
...
6. **暂停确认**：展示幻灯片概览（总页数、各页类型和标题），请用户确认内容完整性后再进入 Stage 2。如用户要求跳过确认则直接推进
...
5. **暂停确认**：展示旁白概览（总字数、预估总时长 = 总字数 ÷ 4 字/秒，抽查首页/中间页/末页旁白），请用户确认内容质量后再进入 Stage 4。如用户要求跳过确认则直接推进
