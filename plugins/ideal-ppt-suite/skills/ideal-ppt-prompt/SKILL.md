...
将幻灯片大纲转换为自包含的 SVG 生成提示词。每个提示词必须独立完整，可直接交给 SVG 生成模型产出最终幻灯片，无需任何外部上下文。
...
1. **确定幻灯片元数据**：
   ...
   - 目标 SVG 文件名（`NN-slide-{slug}.svg`）

2. **构建提示词内容**：

   基于模板 `references/base-prompt.md`，填充以下两个插入点：
   - **STYLE_INSTRUCTIONS**：从 design-spec.md 提取的视觉参数（颜色、字体、排版）
   - **SLIDE CONTENT**：从 outline.md 提取的具体内容（标题、正文、数据、视觉描述）
...
2. 运行 `scripts/generate-manifest.py` 生成 `prompts/svg-generation-manifest.json`
3. 输出执行摘要：
...
共 N 页提示词，全部通过自包含验证。
清单文件：prompts/svg-generation-manifest.json
```
...
| 文档 | 用途 |
|------|------|
| `references/base-prompt.md` | SVG 生成基础提示词模板，包含两个插入点 |
...
| 产物 | 路径 | 说明 |
|------|------|------|
| 提示词文件 | `prompts/NN-slide-{slug}.md` | 每页幻灯片一个，自包含的 SVG 生成提示词 |
| 生成清单 | `prompts/svg-generation-manifest.json` | 所有提示词的索引清单，供 P11 SVG 执行阶段使用 |
...
| P11 SVG 执行 | `prompts/*.md` + `svg-generation-manifest.json` | 逐页读取提示词生成 SVG |
