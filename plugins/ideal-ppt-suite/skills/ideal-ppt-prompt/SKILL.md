...
description: Use when P5 outline review is completed and prompt engineering phase begins. Triggered by ideal-ppt-workflow at P7.
---
...
将幻灯片大纲转换为自包含的 HTML 生成提示词。每个提示词必须独立完整，可直接交给 HTML 生成模型产出最终幻灯片（HTML+CSS+内嵌SVG），无需任何外部上下文。
...
1. 读取 `outline.md`，提取以下内容：
   - **STYLE_INSTRUCTIONS 块**：从大纲文件头部的风格指令区域提取，包含配色、字体、布局规则
   - **幻灯片列表**：每页幻灯片的编号、标题、类型、内容摘要、布局建议、备注
   - **全局参数**：总页数、叙事弧线、风格预设名称

2. 读取 `design-spec.md`，提取以下内容：
   - **视觉主题参数**：主色 / 辅色 / 强调色的 HEX 值
   - **字体系统**：标题字体、正文字体、强调字体及字号层级（CSS font-family 栈）
   - **排版原则**：CSS Grid/Flexbox 布局规则、间距体系、视觉层次
   - **图标使用规格**：图标风格和用法
   - **设计检查清单**：每页必须满足的设计规范

3. 合并两份文件的信息，形成每个提示词所需的完整风格参数集。
...
1. **确定幻灯片元数据**：
   ...
   - 目标 HTML 文件名（`NN-slide-{slug}.html`）

2. **构建提示词内容**：

   基于模板 `references/base-prompt.md`，填充以下两个插入点：
   - **STYLE_INSTRUCTIONS**：从 design-spec.md 提取的 CSS 变量定义（颜色、字体、间距）
   - **SLIDE CONTENT**：从 outline.md 提取的具体内容（标题、卡片、数据、布局指令）
...
| 检查项 | 要求 |
|--------|------|
| 风格参数完整 | 包含完整的 STYLE_INSTRUCTIONS（CSS 变量：颜色/字体/间距全部具体） |
...
2. 运行 `scripts/generate-manifest.py` 生成 `prompts/html-generation-manifest.json`
3. 输出执行摘要：
...
共 N 页提示词，全部通过自包含验证。
清单文件：prompts/html-generation-manifest.json
```
...
| 文档 | 用途 |
|------|------|
| `references/base-prompt.md` | HTML 生成基础提示词模板，包含两个插入点 |
| `references/prompt-engineering.md` | 提示词编写指南，结构规范和正反示例 |
| `references/content-depth-rules.md` | 内容深度规则，反抽象检查清单 |
| `scripts/generate-manifest.py` | 清单生成脚本，扫描 prompts/ 产出 manifest JSON |
...
| 产物 | 路径 | 说明 |
|------|------|------|
| 提示词文件 | `prompts/NN-slide-{slug}.md` | 每页幻灯片一个，自包含的 HTML 生成提示词 |
| 生成清单 | `prompts/html-generation-manifest.json` | 所有提示词的索引清单，供 P11 HTML 执行阶段使用 |
...
| P11 HTML 执行 | `prompts/*.md` + `html-generation-manifest.json` | 逐页读取提示词生成 HTML |
...
处理流程：
  1. 读取 outline.md → 提取 10 页幻灯片内容和 STYLE_INSTRUCTIONS
  2. 读取 design-spec.md → 提取 #005587 / #2196F3 / #FF9800 配色等
  3. 逐页生成提示词：
     - 01-slide-cover.md → 封面：标题 "机器学习平台架构方案"，副标题，日期
     - 02-slide-background.md → 背景：3 个行业数据卡片（具体数值）
     - 03-slide-architecture.md → 架构图：4 层技术栈（每层具体组件名）
     ...
  4. 每页通过内容深度检查
  5. 保存到 prompts/ 目录
  6. 运行 generate-manifest.py → 生成清单

输出：
  prompts/01-slide-cover.md ... prompts/10-slide-next-steps.md
  prompts/svg-generation-manifest.json
```
