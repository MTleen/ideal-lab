...
2. **构建提示词内容（按 rendering_mode 分支）**：
   ...
   - **STYLE_INSTRUCTIONS**：Style Brief — 视觉风格描述，引用预设风格或从维度组合。格式要求包含：
     ...
     ```
     Style Brief:
     - 预设风格：商务风格（corporate）— 干净、专业、几何感
     - 质感：soft-shadow（柔和阴影，微浮层感）
     - 间距：standard（约 12% 画布留白）
     - 主色调：深藏青 #1E3A5F 搭配金色 #C9A227 点缀
     - 背景色：#FFFFFF（纯白）
     - 文字色：#1E3A5F（标题）/ #4A5568（正文）/ #999999（注释）
     - 模块色：mod-red #E53935 / mod-blue #1E88E5 / mod-green #43A047 / mod-orange #FB8C00
     - 字体：标题 32-36px 粗体无衬线 / 正文 14-16px 常规无衬线 / 数据 24-36px 粗体
     - 卡片：白色填充 + 柔和阴影 + 1px 浅色边框 + 圆角
     - 表格：交替行背景，表头主色
     - 图标：简洁线性图标，模块色填充
     ```
   - **SLIDE_CONTENT**：从 outline.md 提取的**全部相关内容**（不裁剪、不过滤、不精选）。将大纲中该页所有材料完整填入，包括所有数据点、所有要点、所有细节。让图像模型自主决定如何排版和取舍。密度等级（density level）可选：`minimal`、`balanced`、`dense`、`ultra-dense`
...
| `references/styles/*.md` | 18 种预设风格完整定义（含颜色 token、字体 token、组件规范、Do/Don't） |
...
| `references/dimensions/presets.md` | 预设映射：18 种预设风格 → 6 维度组合对照表 |
...
输入：
  ...
  outline.md（10 页幻灯片大纲）
  styles/corporate.md（DESIGN.md 格式商务风格定义，含完整 token）

处理流程：
  1. 读取 流程状态.md → rendering_mode=image
  2. 读取 outline.md → 提取 10 页幻灯片完整内容（不裁剪）
  3. 读取 styles/corporate.md → 解析 YAML frontmatter 提取颜色/字体/间距 token，
