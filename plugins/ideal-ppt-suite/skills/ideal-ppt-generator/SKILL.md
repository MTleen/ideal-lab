...
### 问题：抽象内容导致模糊图片
AI 图片生成器不理解抽象概念。如果 prompt 只写标签不写内容，AI 只会画空白方块。必须把每条具体内容写出来。
...
### 检查清单（每页 prompt 生成后自检）
- [ ] 列表项是否每个都有具体内容？（不只是标签）
- [ ] 如果介绍一个机制/流程，是否写出了具体步骤？
- [ ] 如果展示数据，是否有具体的数值和单位？
- [ ] 如果展示对比，是否有明确的对比维度和内容？
- [ ] Visual Description 是否描述了具体的布局和数据排列方式？

### Visual Description 规则
不能写"flow diagram"或"comparison table"这种抽象标签。要写：
- 有几个信息块？每个块里有什么内容？
- 用什么颜色区分？排列方式是什么？
- 具体的数据/文本要在图片中呈现哪些？

```
错误: "Flow diagram showing phases with alternating colors"
正确: "3行卡片网格：第1行蓝色卡片（输入阶段，含数据采集和清洗），第2行绿色卡片（处理阶段，含特征提取和模型训练），第3行紫色卡片（输出阶段，含结果评估）。每个卡片显示阶段名称和2-3个子步骤。"
```

## 图片生成最佳实践

- **逐张生成**，不要用 batch 模式（baoyu-image-gen batch 模式有模型映射 bug）
- **每张间隔 20 秒**，避免触发 API 限流 (503)
- 使用 `--model` 参数显式指定模型，不要依赖 EXTEND.md 的 default_model（batch 模式会忽略）
- 生成失败时自动重试一次后再报告错误
- 使用风格化替代方案处理敏感公众人物
- 通过会话 ID 保持风格一致性
