...
- 最终 PPTX 存在、非空、可打开，页数与已审核幻灯片一致。
- 如果 PDF 属于约定交付物，最终 PDF 存在、非空、可打开，页数与 PPTX 一致。
- 最终交付路径已经确定，不会把预览 PDF 或草稿误认成最终文件。

任一验证失败时，停止交付并保留诊断材料；不得提前清理。

### Step 5: 清理本次任务的临时产物（强制）

最终交付物验证通过后、报告完成前，必须清理本次任务产生且不属于交付范围的临时版本和预览产物。

**应清理：**

- 仅用于校验的预览 PDF、渲染检查 PDF、逐页临时 PDF。
- 已被最终版本替代的 draft、preview、backup、迭代导出文件。
- 临时合并文件、QA 缩略图、截图检查目录和转换工具遗留的临时目录。

**必须保留：**

- 最终 `{topic-slug}.pptx`，以及约定交付的 `{topic-slug}.pdf`。
- 用户提供的源素材、参考 PDF/DOCX/PPTX、模板和其他原始文件。
- 可继续编辑或重新生成交付物所需的正式流程资产：`analysis.md`、`strategy.md`、`outline.md`、`prompts/`、`html_output/`、`images_output/`、`notes/`。
- 来源不明、无法确认是否由本次任务生成的文件。

**安全执行规则：**

1. 先列出候选清单，逐项确认“本次任务生成 + 已有最终替代 + 不在交付范围”。
2. 只对确认后的精确路径执行删除；禁止对项目目录使用无约束通配符或递归清空。
3. 优先使用可恢复的删除方式；若只能永久删除，也必须逐项使用精确路径。
4. 删除后重新检查目录，确认最终交付物和必须保留项仍然存在。
5. 在交付摘要中记录已删除的临时产物；如没有可安全确认的候选项，写明“未清理：来源无法确认”，不得猜测删除。
