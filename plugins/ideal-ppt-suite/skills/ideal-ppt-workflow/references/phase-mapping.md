...
|------|-----|
| **Phase** | P11.5 演讲备注 |
| **Skill** | `ideal-ppt-speaker-notes` |
| **Input Artifacts** | `source-{slug}.{ext}`、`outline.md`、`strategy.md`、`prompts/*.md` |
| **Output Artifacts** | `notes/total.md` |
| **Prerequisites** | P7 completed（P8 approved 或 auto_approved） |
| **BLOCKING** | 否 |
| **可跳过** | `review.skip_notes: true` 时跳过，状态设为 `skipped` |

**说明**：演讲备注属于内容层，与渲染模式完全无关。基于大纲、策略和提示词生成每页的演讲脚本、要点提炼和时长预估。产出 `notes/total.md` 供 P13 导出时嵌入 PPTX speaker notes。不依赖 P11（幻灯片执行），理论上可并行，但为保证编排简单，串行放在 P11 之后。
...
| **Output Artifacts** | *(用户反馈)* |
| **Prerequisites** | P11 completed，P11.5 completed 或 skipped |
| **BLOCKING** | 否 |
...
| **Input Artifacts** | `html_output/*.html`（html 模式）或 `images_output/*.png`（image 模式）、`notes/total.md`（如 P11.5 未跳过） |
| **Output Artifacts** | `{topic-slug}.pptx`、`{topic-slug}.pdf` |
| **Prerequisites** | P11 completed，P11.5 completed 或 skipped，P12 approved |
| **BLOCKING** | 否 |
...
| P7 → P11.5 | P7 completed 且 P8 通过后即可进入 P11.5（不依赖 P11） |
| P9 → P10 | P9 完成后才进入 P10，P9 跳过则 P10 也跳过 |
| P11 → P11.5 | 串行编排：P11 完成后进入 P11.5 |
| P11.5 → P13 | P11.5 completed（或 skipped）后进入 P12，P12 通过后进入 P13 |
...
| P11.5 | `notes/total.md` | `test -f notes/total.md && test -s notes/total.md` |
