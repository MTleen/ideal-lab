...
| **Output Artifacts** | `01-slide-cover.svg`、`02-slide-{name}.svg`、... |
| **Prerequisites** | P7 completed（P8 approved 或 auto_approved） |
| **BLOCKING** | 否 |

**说明**：根据提示词逐页生成最终 SVG 幻灯片。每页 SVG 遵循固定 `viewBox="0 0 1280 720"`，使用真实文本节点，优先使用基础 SVG 图元。如果存在 AI 生成图像（P9），在 SVG 中引用对应图像。
...
| **Phase** | P12 SVG 审核 |
| **Skill** | *(无，用户直接审核)* |
| **Input Artifacts** | `NN-slide-*.svg` |
| **Output Artifacts** | *(用户反馈)* |
| **Prerequisites** | P11 completed |
| **BLOCKING** | 否 |

**说明**：用户审核最终 SVG 幻灯片的视觉效果。如有问题可指定 `--regenerate {N}` 重新生成特定页面。
...
**说明**：将所有 SVG 幻灯片合并导出为 PowerPoint 和 PDF 文件。使用 `merge-to-pptx.ts` 和 `merge-to-pdf.ts` 脚本。默认采用 editable 模式（SVG 解析为 PPT 原生 shape/text）。
...
| P11 | `NN-slide-*.svg`（至少 1 个） | `ls *-slide-*.svg >/dev/null 2>&1` |
