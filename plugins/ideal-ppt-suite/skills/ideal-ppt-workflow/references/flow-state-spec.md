...
rendering_mode: {html|image}                    # 必填，渲染路径
# html  = HTML+CSS 精确布局，可编辑 PPTX
# image = 画图模型自主布局，图片嵌入 PPTX

# === 流程状态 ===
current_phase: P1                               # 必填，当前阶段 P1-P13（含 P11.5）
...
| `rendering_mode` | enum | 是 | 渲染路径：`html`（HTML+CSS 可编辑）或 `image`（画图模型自主布局） |
...
| `review.skip_notes` | boolean | 是 | 是否跳过演讲备注生成（P11.5） |
...
| P11 幻灯片执行 | 产物 | ⏳ pending | - | html_output/*.html 或 images_output/*.png | rendering_mode 决定执行器 |
| P11.5 演讲备注 | 产物 | ⏳ pending | - | notes/total.md | 可通过 review.skip_notes 跳过 |
| P12 幻灯片审核 | 评审 | ⏳ pending | - | - | - |
...
4. 提取 rendering_mode（判断 P7/P11/P13 走哪条路径）
5. 提取 style_reference（rendering_mode=image 时传递给执行器）
6. 提取 image_approach（判断 P9/P10 是否跳过）
7. 提取 review 配置（判断 P6/P8/P10/P12 是否自动通过）
8. 解析 Markdown 状态表
9. 确定第一个非 completed/approved/skipped/auto_approved 的阶段
10. 进入该阶段
```
...
| current_phase 不在 P1-P13（含 P11.5）范围 | 报告非法值，停止流程 |
