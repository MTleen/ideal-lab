...
[![Plugins](https://img.shields.io/badge/plugins-14-blue?style=flat-square)](./plugins)
[![Skills](https://img.shields.io/badge/skills-48-green?style=flat-square)](./plugins)
...
目前包含 **14 个插件**、**48 个 Skill**，覆盖从需求到交付的完整链路。
...
| 插件 | 版本 | 说明 | Skills |
|------|------|------|--------|
| [ideal-dev-workflow](./plugins/ideal-dev-workflow) | 2.1.1 | 15 阶段开发流程：需求 → 方案 → 计划 → 编码 → 测试 → 评审 → 交付 | 13 |
| [ideal-ppt-suite](./plugins/ideal-ppt-suite) | 2.1.0 | PPT 全链路生成：调研 → 策略 → 大纲 → 提示词 → 生成 → 配图 → 导出 | 11 |
...
| [course-to-video](./plugins/course-to-video) | 1.0.0 | 课程转视频全链路：参考资料→课程 Markdown→HTML 幻灯片→TTS 旁白→教学视频 |

### 开发工具

| 插件 | 版本 | 说明 |
|------|------|------|
| [ideal-agent-loop](./plugins/ideal-agent-loop) | 1.1.0 | 目标驱动 Agent Loop（loop engineering）：规划最小闭环 → 持久迭代到完成，支持需求池 goal 接力 |
| [ideal-backlog](./plugins/ideal-backlog) | 1.0.2 | 需求池构建与管理：入队、状态流转、优先级 + FIFO 排序，作为 ideal-agent-loop 出队契约源 |
...
| 插件 | 版本 | 说明 |
|------|------|------|
| [skill-builder](./plugins/skill-builder) | 1.2.1 | Skill 全生命周期引擎：从零创建或爬山优化，7 维度评估 + 棘轮机制 |
| [panel-review](./plugins/panel-review) | 1.1.0 | 多维会审：动态组建专家团队，12 维度攻击面库，对抗性审查文档/代码/方案 |
