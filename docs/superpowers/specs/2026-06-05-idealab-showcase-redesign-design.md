...
│       └── plugin-pain-points.json     # 12 plugin × 2-4 条痛点（spec 阶段已基于 SKILL.md 真实数据起草）
├── public/
│   └── graphs/
...
├────────────────────────────────────────────────────────────┤
│ HERO BAND (h-32)                                            │
│  "46 个 skill · 12 个 plugin · 183 条引用关系"             │
│  一句话价值主张                                              │
├────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────┐  ┌──────────────────────┐    │
│  │   交互式知识图谱         │  │  任务面板（Tasks）    │    │
...
**硬约束**：
- 第一版 12/12 plugin 全部有内容（spec 阶段已基于 SKILL.md 真实数据起草，存 `plugin-pain-points-data.json`，S2 移到 `site/src/data/`）
...
- 允许"这个 Plugin 存在的**原因**"
- **不**写"做了之后业务提升 X%"
- 第一版 12/12 plugin 全部有内容；S2 移入 `site/src/data/plugin-pain-points.json`
...
| **S2 数据层** | `lib/graph.ts`、`lib/tasks.ts`、`lib/plugin-pain-points.ts`、`lib/skill-summary.ts`、`data/tasks.json`（13 个）、`data/plugin-pain-points.json`（12/12 完整，41 条痛点） | build-time 校验全部通过：所有 task 的 skillId 在 graph 中存在；所有 plugin-pain-points 的 slug 是真实 plugin |
...
| 痛点卡内容"看起来像编的" | 中 | 低 | 12/12 plugin 已基于 SKILL.md 真实数据起草，41 条；S2 仅做格式校验和搬运 |
...
| 痛点卡第一版完整度 | ✅ 12/12 plugin 全部有内容（41 条） | spec 阶段已完成数据起草；见 `plugin-pain-points-data.json` |
